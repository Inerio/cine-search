import { Component, inject, ChangeDetectionStrategy } from '@angular/core';
import { RouterLink } from '@angular/router';
import { LangSwitchComponent } from '../lang-switch/lang-switch.component';
import { TranslationService } from '../../services/translation.service';

@Component({
  selector: 'app-header',
  standalone: true,
  imports: [RouterLink, LangSwitchComponent],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <header class="header">
      <a routerLink="/" class="logo">
        <span class="logo-icon">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <circle cx="11" cy="11" r="8"/>
            <path d="m21 21-4.35-4.35"/>
            <path d="m8 11 2 2 4-4"/>
          </svg>
        </span>
        <span class="logo-text">Movie<span class="accent">Seeker</span></span>
      </a>

      <nav class="nav-links">
        <a routerLink="/" class="nav-link">{{ t('header.home') }}</a>
        <a routerLink="/search" class="nav-link">{{ t('header.explore') }}</a>
        <a routerLink="/search" [queryParams]="{ tab: 'scene' }" class="nav-link">{{ t('header.advanced') }}</a>
      </nav>

      <div class="spacer"></div>

      <a href="https://buymeacoffee.com/inerio" target="_blank" rel="noopener noreferrer" class="coffee-link" title="Buy me a coffee">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <path d="M17 8h1a4 4 0 1 1 0 8h-1"/>
          <path d="M3 8h14v9a4 4 0 0 1-4 4H7a4 4 0 0 1-4-4Z"/>
          <line x1="6" y1="2" x2="6" y2="4"/>
          <line x1="10" y1="2" x2="10" y2="4"/>
          <line x1="14" y1="2" x2="14" y2="4"/>
        </svg>
        <span class="coffee-text">{{ t('header.coffee') }}</span>
      </a>

      <app-lang-switch />
    </header>
  `,
  styleUrl: './header.component.scss'
})
export class HeaderComponent {
  private ts = inject(TranslationService);

  t(key: string): string { return this.ts.t(key); }
}
